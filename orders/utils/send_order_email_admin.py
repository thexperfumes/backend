...
def send_order_email_to_admin(order):
    try:
        ...
        email = EmailMessage(
            subject=f"Test Admin Mail - {order.invoice_number}",
            body="This is test admin mail.",
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=[admin_email],
        )

        sent_count = email.send(fail_silently=True)
        logger.info(f"Admin send count: {sent_count}")
        return sent_count == 1

    except Exception as e:
        logger.exception(f"Admin test mail failed: {e}")
        return False        